...
treatment_assign <- function(data,
    ...
    objetos<-list("summary_strata" = resumen_strata, "data" = data %>% select(key, strata, treat))
...
treatment_assign1 <- function(data,
                             share_control,
                             n_t = 2,
                             strata_varlist,
                             missfits = "global",
    ...
    data <- 
        ...
        dplyr::select(-c(missfit_strata, num_missfits, row_missfit))
