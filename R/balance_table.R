...
balance_table<-function(data, treatment) {
  ...
  bal_tables<-purrr::map(trats, function (x)
          dplyr::filter(data, !!rlang::sym(treatment) == valores_trat[1] | !!rlang::sym(treatment) ==  !!x))
  
  bal_tables<-purrr::map(bal_tables, function (x) 
                    tidyr::pivot_longer(x, names_to = "variables", values_to = "value", -treatment) )
  ...
  bal_tables<-purrr::map(bal_tables, function(x) x %>% 
                    ...
                    dplyr::summarise(Media_control = stats::t.test(value~treatment)$estimate[1],
                              Media_trat = stats::t.test(value~treatment)$estimate[2], 
                              p_value = stats::t.test(value~treatment)$p.value))
